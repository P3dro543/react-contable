// ============================================================
// ReporteTercerosPage.jsx — Reporte de movimientos por Tercero (AUX9)
// API: GET /reportes/terceros
// Params: id_tercero (req) + (id_periodo | fecha_inicio+fecha_fin) + id_estado?, pagina, por_pagina
...
// Estados de asiento como los maneja el backend (id numérico)
// Si el back filtra por id_estado numérico, mostramos el nombre pero enviamos el id
const ESTADOS = [
  { label: "— Todos —", value: "" },
...
export function ReporteTercerosPage() {
  const [terceros, setTerceros]     = useState([]);
  const [modoFecha, setModoFecha]   = useState("rango");
  const [filtros, setFiltros]       = useState({
    id_tercero: "", fecha_inicio: "", fecha_fin: "",
    id_periodo: "", id_estado: "", pagina: 1, por_pagina: 10,
  });
  const [resultado, setResultado]   = useState(null);  // { tercero, data, total }
  ...
  // Cargar terceros para el select
  useEffect(() => {
    terceroService.listar(1, 200)
      .then((d) => setTerceros(d.rows || []))
      .catch(() => {});
  }, []);
  ...
  const validar = () => {
    if (!filtros.id_tercero) return "Debe seleccionar un tercero.";
  ...
  const buscar = async (paginaOverride) => {
    ...
    try {
      // Construir params limpios según modo de fecha
      const params = {
      ...
      const data = await reporteService.reporteTercero(params);
      setResultado(data);
    } catch (err) {
  ...
  const rows  = resultado?.rows  || resultado?.data  || [];
  const total = resultado?.total || 0;

  return (
    <Layout title="Reporte por Tercero">
      <div className="page-header">
        <div>
          <h2 className="page-title">Reporte por Tercero</h2>
          <p className="page-subtitle">Movimientos de asientos asociados a un tercero — AUX9</p>
        </div>
      </div>
      ...
      <div className="filters-card">
        ...
        <div className="filters-grid">
          ...
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Tercero <span className="required">*</span></label>
            <select
              className="form-control"
              value={filtros.id_tercero}
              onChange={(e) => setF("id_tercero", e.target.value)}
            >
      ...
      {buscado && (
        <div className="card">
          {/* Info del tercero seleccionado */}
          {resultado?.tercero && (
            <div className="card-header">
              <span className="card-title">
                📋 {resultado.tercero.nombre}
              </span>
                  ...
                  {rows.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ fontFamily: "var(--mono)", textAlign: "right", fontWeight: 500 }}>
                        {formatMonto(r.monto_prorrateo ?? r.monto)}
                      </td>
                      <td>
          ...
          {rows.length > 0 && (
            <div className="total-row">
              ...
              <span className="total-amount">{formatMonto(resultado?.total_monto ?? resultado?.suma ?? null)}</span>
            </div>
          )}
