...
export function ProrrateoCCPage() {
  ...
  const fetchCostCenters = async () => {
    try {
      const response = await axiosInstance.get('/centrocostos');
      // Pedro para centros de costo devuelve la lista directa o en rows
      const ccList = response.data.rows || response.data.data || response.data;
      if (Array.isArray(ccList)) {
        const formatted = ccList.map(cc => ({
          id: cc.id_centro_costo,
  ...
  const toggleAsiento = async (asientoId) => {
    ...
    if (!asientoDetails[asientoId]) {
      try {
        ...
        console.error("Error al cargar detalle de asiento", err);
      }
    }
  };
  ...
  const openProrrateoModal = async (line) => {
    try {
      const response = await axiosInstance.get(`/api/prorrateo/centros-costo/${line.id_detalle}`);
  ...
  const handleSaveProrrateo = async (distribution) => {
    try {
      ...
      if (response.status === 200) {
        setToast({ message: "Prorrateo guardado correctamente", type: "success" });
        setModalConfig({ isOpen: false, lineData: null, initialData: [] });
      }
    } catch (err) {
      setToast({ message: "Error al guardar prorrateo", type: "error" });
    }
  };
  ...
  return (
    <Layout title="Asignación de Centros de Costo">
      <div className="prorrateo-page">
        <div className="page-header">
          <div>
            <h1 className="page-title">Asignación de Centros de Costo</h1>
            <p className="page-subtitle">Distribuya montos entre los diferentes centros de costo del sistema.</p>
          </div>
        </div>
        ...
        {modalConfig.isOpen && (
          <ProrrateoModal
            ...
            title={`Distribución - ${modalConfig.lineData.descripcion}`}
